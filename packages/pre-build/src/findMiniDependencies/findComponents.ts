...
const parsedJsonMap = new Map<string, Component[]>();

export default function findComponents(jsonPath: string, projectDir: string) {
  if (parsedJsonMap.has(jsonPath)) {
    return parsedJsonMap.get(jsonPath)!;
  }
  ...
  for (const key in usingComponents) {
    ...
    if (!fs.existsSync(item.jsPath)) {
      throw new Error(`Can not find the component \`${item.configPath}\` in config file: \`${item.configFilePath}\`.`);
    }
  ...
  parsedJsonMap.set(jsonPath, result);
  return result;
}
