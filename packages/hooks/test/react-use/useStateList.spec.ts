import useStateList from 'react-use/esm/useStateList';
...
describe('useStateList', () => {
  ...
  function getHook(list: any[] = ['a', 'b', 'c']) {
    return renderHook(props => useStateList(props?.states), { initialProps: { states: list } });
  }
  ...
  describe('setState()', () => {
    it('should set state value if it exists in states list', async () => {
      const hook = getHook();
  ...
  describe('setStateAt()', () => {
    it('should set state by it`s index in states list', async () => {
      const hook = getHook();
    ...
    it('should cyclically travel through the right border', async () => {
      const hook = getHook();
    ...
    it('should cyclically travel through the left border', async () => {
      const hook = getHook();
  ...
  describe('next()', () => {
    it('should switch states forward and cause re-render', async () => {
      const hook = getHook();
  ...
  describe('prev()', () => {
    it('on overflow should switch to last element (should be cycled)', async () => {
      const hook = getHook();
    ...
    it('should switch states backward and cause re-render', async () => {
      const hook = getHook();
  ...
  describe('on state list shrink', () => {
    it('should set last element as state if index was beyond new last element', async () => {
      const hook = getHook();
      act(() => {
        hook.result.current.prev();
      });
    ...
    it('should so nothing if current index within new range', async () => {
      const hook = getHook();
      act(() => {
        hook.result.current.prev();
      });
