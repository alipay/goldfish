...
it('should have an added key', async () => {
...
it('should remove an existing key on toggle', async () => {
  const { result } = setUp(new Set([1, 2]));
  const [, utils] = result.current;
...
it('should add a new key on toggle', async () => {
...
it('should reset to initial set provided', async () => {
  const { result } = setUp(new Set([1]));
  const [, utils] = result.current;
