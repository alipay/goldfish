...
import useList, { ListActions } from 'react-use/esm/useList';
...
describe('useList', () => {
  ...
  describe('updateFirst()', () => {
    it('should replace first items that matches the predicate and cause re-render', async () => {
      ...
      expect(hook.result.current[0]).toBe(1);
      act(() => {
        updateFirst((a: any, _: any) => a % 2 === 1, 0);
      });
      await timeout();
      expect(hook.result.current[0]).toBe(2);
  ...
  describe('sort()', () => {
    it('should sort the list with given comparator and cause re-render', async () => {
      ...
      expect(hook.result.current[0]).toBe(1);
      act(() => {
        sort((a: any, b: any) => (a === b ? 0 : a < b ? 1 : -1));
      });
      await timeout();
      expect(hook.result.current[0]).toBe(2);
