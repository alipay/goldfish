...
it('should increment by 1 if not value received', async () => {
  const { result } = setUp(5);
...
it('should increment by value received', async () => {
  const { result } = setUp(5);
  const { get, inc } = result.current[1];
...
it('should decrement by 1 if not value received', async () => {
  const { result } = setUp(5);
...
it('should decrement by value received', async () => {
  const { result } = setUp(5);
  const { get, dec } = result.current[1];
...
it('should set to value received', async () => {
  const { result } = setUp(5);
...
it('should reset and set new original value', async () => {
  const { result } = setUp(5);
  const { get, set, reset } = result.current[1];
