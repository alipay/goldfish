...
import useStateValidator, { StateValidator, UseStateValidatorReturn } from 'react-use/esm/useStateValidator';
...
describe('useStateValidator', () => {
  ...
  it('should return an array of two elements', async () => {
    const [, hook] = getHook();
    await timeout();
  ...
  it('first element should represent current validity state', async () => {
    const [, hook] = getHook();
    await timeout();
  ...
  it('validator have to be called on init plus on each state update', async () => {
    ...
    await timeout();
    expect(spy).toHaveBeenCalledTimes(2);
    act(() => setState((prevState: any) => prevState + 1));
    await timeout();
    expect(spy).toHaveBeenCalledTimes(3);
  });

  it('should pass to validator one parameter - current state', async () => {
    const [spy, hook] = getHook(jest.fn());
    const [setState] = hook.result.current;
  ...
  it('if validator expects 2nd parameter it should pass a validity setter there', async () => {
    const [spy, hook] = getHook(
