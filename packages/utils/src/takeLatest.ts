 ...
 */
function takeLatest<FuncType extends PromiseCreator>(promiseCreator: FuncType): FuncType {
  let index = 0;
  return ((...args) => {
    index += 1;
    const promise = promiseCreator(...args);

    function latest(func: (value?: any | PromiseLike<any>) => void, reqIndex: number) {
